...
1. **Scope & Analyze** – Scout target areas with `code_mapper` agents, then use `context_builder` with `response_type: "review"` informed by their findings.
...
### 1a. Scout the territory with `code_mapper` agents

Before calling `context_builder`, dispatch `code_mapper` agents to map the areas the user wants refactored. A quick `get_file_tree` or `file_search` orients you, then spawn 2-3 agents for the most relevant areas. Use `reasoning_effort:"low"` and `fork_turns:"none"` for these narrow mapping probes:
...
⚠️ Don't skip this step. Use the scout agents' findings to write a well-informed `context_builder` call with `response_type: "review"`:
...
- 🚫 Forgetting to check on dispatched agents — they may block on permission approvals; use `wait_agent` / `list_agents` periodically to keep them unblocked
