...
- **Still ambiguous after 2 calls** -> dispatch a narrow `code_mapper` agent with one specific question.
...
Mapping agents are cheap -- spawn multiple in parallel for different areas, but keep each prompt narrow. They tend to overthink broad instructions.
...
The tool returns `oracle_export_path` and `oracle_export_instruction`. Include `oracle_export_path` inside the `message` you send on your next `spawn_agent` call. The `oracle_export_instruction` field is a ready-made sentence ("Read the Oracle export at `<path>` with `read_file` ...") you can emit verbatim at the head of that `message`. The child agent opens the file with `read_file`. Do **not** ask child agents to continue your Oracle chat -- they are in different agent threads.
...
When steering, the loop is the same but step 5 becomes `followup_task` on the existing `task_name` instead of a fresh dispatch:

```json
{"tool":"followup_task","args":{
	"target":"orchestrate_1",
...
- **`implementation_lead`** (`reasoning_effort: "high"`) -- The default for complex work. Owns a bounded implementation branch and may spawn child agents for mapping, docs, tests, browser debugging, or review.
- **`investigation_lead`** (`reasoning_effort: "high"`) -- Complex diagnosis where several focused probes may be needed before a fix is clear.
- **`code_mapper`** (`reasoning_effort: "low"`) -- Read-only reconnaissance, ownership maps, call paths, wiring, and ambiguity collapse.
- **`debugger`** (`reasoning_effort: "high"`) -- Read-only root-cause isolation from failures, logs, stack traces, flaky behavior, or broken tests.
- **`test_automator`** (`reasoning_effort: "medium"`) -- Focused regression tests, harness work, and validation commands.
- **`reviewer`** (`reasoning_effort: "high"`) -- PR-style correctness, regression, security, edge-case, and missing-test review.
- **`docs_researcher`** (`reasoning_effort: "low"`) -- Current official documentation/API/library behavior checks.
- **`browser_debugger`** (`reasoning_effort: "medium"`) -- Browser/UI reproduction with console, network, DOM, and screenshot evidence.
- **`agent_organizer`** (`reasoning_effort: "medium"`) -- Disposable decomposition consultant when the delegation plan itself is unclear.
- **`workflow_orchestrator`** (`reasoning_effort: "high"`) -- Multi-wave workflow planner for complex work with dependencies and wait points.
...
When in doubt, use `implementation_lead`. The tasks reaching this workflow are complex by nature. Use a specialist leaf agent only when the item is narrow enough for that role.

When questions arise during coordination, reason through them yourself. If you're uncertain, negotiate with the agent already working on the relevant task -- it has the deepest context. Use `followup_task` with your thinking and work toward consensus rather than dictating a direction.
...
{"tool":"spawn_agent","args":{"task_name":"orchestrate_a","agent_type":"implementation_lead","reasoning_effort":"high","fork_turns":"none","message":"<brief A>"}}
{"tool":"spawn_agent","args":{"task_name":"orchestrate_b","agent_type":"test_automator","reasoning_effort":"medium","fork_turns":"none","message":"<brief B>"}}

// Then wait for mailbox updates and inspect live agents
{"tool":"wait_agent","args":{}}
{"tool":"list_agents","args":{}}
```

Handle the finished agent, then keep waiting on remaining live agents. While waiting, summarize completed work or prepare the next brief -- be a pipeline, not a sequential loop.
...
Mapping/debugging/docs sessions are good to close right away when they are narrow reconnaissance with no follow-up value. Keep heavier agents open if you might revisit them.
...
| Operation | Tool call |
|-----------|-----------|
| Start a fresh agent | `spawn_agent task_name="<task_name>" agent_type=<shipped_agent_type> reasoning_effort="<role_effort>" fork_turns="none" message="..."` |
| Steer an existing agent | `followup_task target="<task_name>" message="..."` |
| Send a clarification without triggering a turn | `send_message target="<task_name>" message="..."` |
| Wait for agent mailbox updates | `wait_agent` |
| Inspect live agents | `list_agents` |
...
- Extended code reading before delegating -- a quick skim is fine; deep reads belong in builder or shipped mapper/debugger/research agents
...
- Forgetting to check on dispatched agents -- poll periodically with `wait_agent` and `list_agents` to keep them unblocked
