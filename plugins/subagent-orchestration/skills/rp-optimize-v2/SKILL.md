...
- **You (the agent)**: Orchestrate. Translate the prompt, fan out `code_mapper` agents to map surface area, run `context_builder` to plan setup, dispatch `implementation_lead` to land instrumentation + baseline, then loop `implementation_lead` dispatches for each optimization. Your context stays lean.
- **Mapping agents** (`spawn_agent` with `agent_type:"code_mapper"`, `reasoning_effort:"low"`, `fork_turns:"none"`): Read-only sub-agents that map the surface — find AGENTS.md, locate hot paths, discover existing benchmarks, define scope boundaries. Cheap and parallel; spawn liberally during Phase 1.
...
- **Implementation agents** (`spawn_agent` with `agent_type:"implementation_lead"`, `reasoning_effort:"high"`, `fork_turns:"none"`): Carry out implementation, measurement, and hardening. Phase 3 lands instrumentation and the baseline. Each loop iteration lands one attributed change, runs tests, re-measures, and appends to the scoreboard.
...
---
## Phase 1: Surface Mapping & Bottleneck Scouting (delegate to `code_mapper` agents)

Your job here is **prompt translation + orchestrated scouting**, not codebase exploration. Spend at most 1–2 navigation calls turning the user's request into the codebase's actual nouns, then **fan out `code_mapper` agents in parallel** to scout for bottleneck candidates around the named target.
...
- **Profile data already exists** (user attached a sample report or pointed at a recent profiler trace in the repo) → dispatch one focused `code_mapper` to read the trace + summarize bottleneck candidates, then go to Phase 2. Skip the rest of the fan-out.
...
### 1b. Dispatch `code_mapper` agents in parallel

Spawn `code_mapper` agents — each with one narrow question — for the facts you need before `context_builder` can plan. **The bottleneck-candidates scout is the heart of this phase.** Typical fan-out:
...
Each `spawn_agent` call starts an independent agent; no detach flag is needed:

```json
// The Bottleneck candidates scout — full prose, since this is the heart of Phase 1
{"tool":"spawn_agent","args":{
	"task_name":"bottleneck_candidates_<area>",
	"agent_type":"code_mapper",
	"reasoning_effort":"low",
	"fork_turns":"none",
	"message":"Scout for performance bottlenecks around <translated target>. Look at the target AND its surrounding context: callers (especially loops or hot paths invoking it frequently), data dependencies (how inputs are constructed upstream), adjacent operations in the same code path, shared infrastructure the target touches. Hunt for: tight loops with per-iteration allocations, redundant computation, locking/serialization, expensive transformations, sync I/O on hot paths, O(n^2) patterns, unbatched updates. Report 2-3 ranked candidates with file:line refs and a one-sentence rationale per candidate. Don't propose fixes yet."
}}

// One more representative one — same shape, different question
{"tool":"spawn_agent","args":{
	"task_name":"conventions_agents_md",
	"agent_type":"code_mapper",
	"reasoning_effort":"low",
	"fork_turns":"none",
...
// Repeat the same shape for the remaining 3 scouts in the table above
// (Target & call graph, Prior perf work, Scope).

// Then wait on the batch
{"tool":"wait_agent","args":{"timeout_ms":180000}}
...
If the bottleneck-candidates scout returns thin or generic results ("nothing obviously expensive"), that's a signal — either the area is genuinely well-tuned and the user's complaint is elsewhere, or the scout needed broader radius. Either way, **re-dispatch one targeted `code_mapper`** with a wider radius (e.g., "look two call levels up") rather than reading the code yourself.
...
## Phase 3: Land Instrumentation + Baseline (delegate to `implementation_lead`)

You don't run measurements. Dispatch a single `implementation_lead` agent to execute the setup plan:

```json
{"tool":"spawn_agent","args":{
	"task_name":"optimize_setup",
...
When the implementation agent returns:

- **Read the scoreboard.** That's your verification surface — not the diff. If the baseline row looks reasonable, move on.
- **Sanity-check variance.** If the variance band is large enough to swallow a 10–20% optimization, that's a problem. Steer the implementation agent to take more samples or narrow the workload before continuing.
- **Don't read the instrumentation diff yet.** If concerns surface in later phases, dispatch a narrow `code_mapper` to summarize the diff for you.
...
Mapping-agent sessions are good to dismiss right away — narrow reconnaissance, no follow-up value. Keep heavier agent sessions if you might revisit them.
...
Dispatch **one `implementation_lead` agent** for the selected change. The brief covers landing the optimization **and** hardening it in one shot — implementation, tests, re-measurement, scoreboard append:

```json
{"tool":"spawn_agent","args":{
	"task_name":"optimize_<n>",
	"agent_type":"implementation_lead",
	"reasoning_effort":"high",
	"fork_turns":"none",
...
Always use `implementation_lead` with `reasoning_effort:"high"`. Optimizations involve trade-offs (correctness, locality, complexity), and re-measurement requires interpreting noisy results. Use `test_automator` with `reasoning_effort:"medium"` only for isolated measurement/test work.
...
- **Tests actually ran?** "Ran the tests" in a summary isn't the same as tests passing. If you're suspicious, ask the agent for the exact command and exit code, or dispatch a narrow `test_automator` to re-run them.
- **Behavior spot-check, when needed.** If the change touches a correctness-sensitive surface, dispatch a narrow `code_mapper` agent to read the diffed files and report whether semantics shifted (early returns, cache invalidation, ordering). Don't pull the diff into your own context unless the scout flags something concrete.
...
- **"Can't tell — measurement is too noisy" or "behavior may have regressed"** → **stop optimizing.** The next dispatch must be an `implementation_lead` to harden the instrumentation or fix the regression. Do not plan a new optimization on top of an unreliable measurement — every later result becomes uninterpretable.
...
The loop is serial by design — attribution collapses when you run multiple changes at once. The one legitimate use of parallelism is **evaluating alternatives** for the same slot: dispatch two `implementation_lead` agents to try two different candidate optimizations on branches or temporary copies, pick the winner, discard the loser. This is advanced and rarely worth the coordination cost; don't reach for it unless the oracle explicitly suggests it.
...
| Capability | Mapping Agents | `context_builder` | Implementation Lead (setup) | Implementation Lead (each loop) | Oracle (`oracle_send`) |
|---|---|---|---|---|---|
...
```
spawn_agent agent_type=<code_mapper|implementation_lead> reasoning_effort=<effort> fork_turns=none  # dispatch
wait_agent / list_agents                                                                  # block/check
followup_task target="<task_name>"                                                        # correct
...
- 🚫 Skipping the bottleneck-candidates scout because the user named a specific function — even with a named target, callers and inputs often dominate the cost
- 🚫 Skipping `context_builder` in Phase 2 and dispatching the setup implementation agent from your own sketch — you'll lose the candidate queue and the instrumentation gating discipline
- 🚫 Letting `context_builder` re-derive optimization candidates from scratch instead of seeding it with the bottleneck candidates from Phase 1 — the scout already paid for that scouting; pass it forward
